...
| Key Vault: `retrieving contact for KeyVault ... context deadline exceeded` ao criar o vault | A criação do vault com `enable_rbac_authorization = true` já lê os contatos de certificado no plano de dados; sem role de dados do Key Vault já propagada, isso falha 100% das vezes (não é falha de rede) | Já corrigido no `keyvault.tf`: a role `Key Vault Administrator` agora é atribuída no Resource Group **antes** do vault existir, com `time_sleep` de 60s antes de criar o vault. Se ainda ocorrer, rode `terraform apply` de novo |
| Storage: `FeatureNotSupportedForAccount: tierToArchive is not supported for the account` | Causa ainda não confirmada nesta conta (não é ZRS, é `StorageV2`/`LRS` — as causas documentadas pela Microsoft não batem). Pode ser uma Azure Policy da assinatura Azure for Students forçando outro tipo de redundância nos bastidores | Diagnosticar: `az storage account show -n <storage> -g <rg> --query "{kind:kind,sku:sku,isHns:isHnsEnabled}"`. Se bloquear o lab, remova temporariamente `tier_to_archive_after_days_since_modification_greater_than` da regra em `storage.tf` (mantém Hot→Cool e delete, só perde a demonstração de Archive) |
