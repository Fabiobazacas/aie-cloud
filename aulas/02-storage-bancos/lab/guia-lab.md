...
> **Por que `time_sleep` antes do vault (e não só antes do segredo)?** RBAC tem ~30-60s de propagação. A role (`Key Vault Administrator`) é atribuída no **Resource Group**, antes de o vault existir — porque a própria criação de um vault com `enable_rbac_authorization = true` já lê os "contatos de certificado" no plano de dados, e sem a role já propagada isso falha 100% das vezes com `context deadline exceeded` (não um erro de permissão comum — o provider trata a falta de permissão nesse endpoint como timeout). O `depends_on` no vault e nos segredos garante que ambos esperam a mesma propagação.
...
| AI Search: `ServiceQuotaExceeded` / limite de SKU Free atingido | 1 search service Free por **subscription** (não por RG). Comum sobrar um órfão de um `apply` anterior que falhou em outro recurso e nunca foi destruído | `az group list --query "[?starts_with(name,'rg-qc-aula02')].name" -o tsv` para achar o RG antigo, depois `az search service delete --name <nome-antigo> -g <rg-antigo> --yes` (ou apague o RG inteiro com `az group delete -g <rg-antigo> --yes --no-wait`) |
...
| Python: `DefaultAzureCredential failed ... Timeout waiting for token from portal` | No Cloud Shell, `ManagedIdentityCredential` detecta (errado) que há uma Managed Identity disponível, tenta buscar token no "token broker" do Cloud Shell e trava — e o `DefaultAzureCredential` do Python não cai para `AzureCliCredential` (que é o que realmente funciona ali, via `az login`) | Já corrigido nos scripts: `DefaultAzureCredential(exclude_managed_identity_credential=True)` |
| Key Vault: "Forbidden — the user does not have ... action" | RBAC ainda não propagou | `sleep 60` e tentar de novo |
| Key Vault: `retrieving contact for KeyVault ... context deadline exceeded` (`StatusCode=0`) ao criar o vault | `StatusCode=0` indica que a chamada nunca recebeu resposta HTTP (não é 403 de permissão) — algo na rede/latência até `<vault>.vault.azure.net` ou um timeout interno curto do provider para essa chamada específica. RBAC já propagado e `time_sleep` não resolvem (testado). Causa raiz ainda não confirmada nesta assinatura | Em investigação — não ignore, colete diagnóstico: logo após o `apply` falhar (o vault já foi criado, só a leitura de contatos falhou), rode `az keyvault show --name <vault> --query "{publicNetworkAccess:properties.publicNetworkAccessEnabled, networkAcls:properties.networkAcls}"` e `curl -s -o /dev/null -w "%{http_code} %{time_total}s\n" --max-time 15 https://<vault>.vault.azure.net/certificates/contacts?api-version=7.4`. Um `curl` que trava até o `--max-time` confirma bloqueio de rede; uma resposta rápida (mesmo 401) aponta para o provider/SDK |
| Storage: `FeatureNotSupportedForAccount: tierToArchive is not supported for the account` | Causa não confirmada nesta assinatura (a conta é `StorageV2`/`LRS` — as causas documentadas pela Microsoft, como ZRS ou hierarchical namespace, não se aplicam aqui). Possível Azure Policy da assinatura Azure for Students restringindo o tier Archive. Não é atraso de propagação: testado com `time_sleep` de 30s antes da policy, mesmo erro | Já removido do `storage.tf` (regra mantém só Hot→Cool e delete). Se quiser confirmar a causa: `az storage account show -n <storage> -g <rg> --query "{kind:kind,sku:sku,isHns:isHnsEnabled}"` |
